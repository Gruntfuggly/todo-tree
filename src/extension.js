...
function activate(context) {
    ...
    var statusBarIndicator = vscode.window.createStatusBarItem(
        vscode.StatusBarAlignment.Left,
        0
    );

    var todoTreeView = vscode.window.createTreeView('todo-tree-view', {
    ...
    ignoreMarkdownUpdate = context.globalState.get(
        'ignoreMarkdownUpdate',
        false
    );
    ...
    function resetOutputChannel() {
        ...
        if (
            vscode.workspace.getConfiguration('todo-tree.general').debug ===
            true
        ) {
    ...
    function resetFileSystemWatcher() {
        ...
        if (config.scanMode() === SCAN_MODE_WORKSPACE_AND_OPEN_FILES) {
            if (
                vscode.workspace.getConfiguration('todo-tree.general')
    ...
    function updateStatusBarAndTitleBar() {
        var statusBar =
            vscode.workspace.getConfiguration('todo-tree.general').statusBar;
        var fileFilter;
        ...
        if (statusBar === STATUS_BAR_TOTAL) {
            ...
            if (statusBar === STATUS_BAR_CURRENT_FILE) {
                statusBarIndicator.tooltip =
                    'Todo-Tree tags counts in current file';
    ...
    function getOptions(filename) {
        ...
        var submoduleExcludeGlobs =
            context.workspaceState.get('submoduleExcludeGlobs') || [];
        ...
        if (
            ...
            !fs.existsSync(context.storageUri.fsPath)
        ) {
            debug(
                'Attempting to create local storage folder ' +
                    context.storageUri.fsPath
            );
        ...
        options.multiline =
            utils.getRegexSource().indexOf('\\n') > -1 ||
            c.get('regex.enableMultiLine') === true;

        if (
            fs.existsSync(context.storageUri.fsPath) === true &&
            c.get('ripgrep.usePatternFile') === true
        ) {
            var patternFileName =
                crypto.randomBytes(6).readUIntLE(0, 6).toString(36) + '.txt';
            options.patternFilePath = path.join(
                context.storageUri.fsPath,
                patternFileName
            );
        }
    ...
    function searchWorkspaces(searchList) {
        ...
        if (
            ...
            scanMode === SCAN_MODE_WORKSPACE_ONLY
        ) {
            var includes = vscode.workspace
                .getConfiguration('todo-tree.filtering')
                .get('includedWorkspaces', []);
            var excludes = vscode.workspace
                .getConfiguration('todo-tree.filtering')
                .get('excludedWorkspaces', []);
    ...
    function applyGlobs() {
        var includeGlobs = vscode.workspace
            .getConfiguration('todo-tree.filtering')
            .get('includeGlobs');
        var excludeGlobs = vscode.workspace
            .getConfiguration('todo-tree.filtering')
            .get('excludeGlobs');
        ...
        if (
            includeGlobs.length +
                excludeGlobs.length +
                tempIncludeGlobs.length +
                tempExcludeGlobs.length >
            0
        ) {
            ...
            search(
                ...
                searchList.length > 0
                    ...
                    : function () {
                          ...
                          if (
                              vscode.workspace
                                  .getConfiguration('todo-tree.ripgrep')
                                  .get('passGlobsToRipgrep') !== true
                          ) {
                              applyGlobs();
                          }
                          addResultsToTree();
    ...
    function getRootFolders() {
        var rootFolders = [];
        var valid = true;
        var rootFolder = vscode.workspace
            .getConfiguration('todo-tree.general')
            .get('rootFolder');
    ...
    function rebuild() {
        ...
        if (config.shouldIgnoreGitSubmodules()) {
            ...
            searchList.forEach(function (rootPath) {
                submoduleExcludeGlobs = submoduleExcludeGlobs.concat(
                    utils.getSubmoduleExcludeGlobs(rootPath)
                );
            });
    ...
    function setButtonsAndContext() {
        var c = vscode.workspace.getConfiguration('todo-tree');
        var isTagsOnly = context.workspaceState.get(
            'tagsOnly',
            c.get('tree.tagsOnly', false)
        );
        var isGroupedByTag = context.workspaceState.get(
            'groupedByTag',
            c.get('tree.groupedByTag', false)
        );
        var isGroupedBySubTag = context.workspaceState.get(
            'groupedBySubTag',
            c.get('tree.groupedBySubTag', false)
        );
        ...
        var showGroupBySubTagButton =
            c.get('tree.buttons').groupBySubTag === true;
        ...
        vscode.commands.executeCommand(
            'setContext',
            'todo-tree-show-scan-mode-button',
            showScanModeButton
        );
        vscode.commands.executeCommand(
            'setContext',
            'todo-tree-show-view-style-button',
            showViewStyleButton
        );
        vscode.commands.executeCommand(
            'setContext',
            'todo-tree-show-group-by-tag-button',
            showGroupByTagButton
        );
        vscode.commands.executeCommand(
            'setContext',
            'todo-tree-show-group-by-sub-tag-button',
            showGroupBySubTagButton
        );
        vscode.commands.executeCommand(
            'setContext',
            'todo-tree-show-filter-button',
            showFilterButton
        );
        vscode.commands.executeCommand(
            'setContext',
            'todo-tree-show-refresh-button',
            showRefreshButton
        );
        vscode.commands.executeCommand(
            'setContext',
            'todo-tree-show-expand-button',
            showExpandButton
        );
        vscode.commands.executeCommand(
            'setContext',
            'todo-tree-show-export-button',
            showExportButton
        );
        ...
        vscode.commands.executeCommand(
            'setContext',
            'todo-tree-grouped-by-tag',
            isGroupedByTag
        );
        vscode.commands.executeCommand(
            'setContext',
            'todo-tree-grouped-by-sub-tag',
            isGroupedBySubTag
        );
        vscode.commands.executeCommand(
            'setContext',
        ...
        vscode.commands.executeCommand(
            ...
            vscode.workspace.getConfiguration('explorer').compactFolders ===
                true
        );
        vscode.commands.executeCommand(
            'setContext',
            'todo-tree-has-sub-tags',
            hasSubTags
        );
    ...
    function hideTreeIfEmpty() {
        ...
        if (
            vscode.workspace
                .getConfiguration('todo-tree')
                .get('tree.hideTreeWhenEmpty') === true
        ) {
            vscode.commands.executeCommand(
                'setContext',
                'todo-tree-is-empty',
    ...
    function isIncluded(uri) {
        if (uri.fsPath) {
            ...
            var excludeGlobs = vscode.workspace
                .getConfiguration('todo-tree.filtering')
                .get('excludeGlobs');
            var includeHiddenFiles = vscode.workspace
                .getConfiguration('todo-tree.filtering')
                .get('includeHiddenFiles');

            var tempIncludeGlobs =
                context.workspaceState.get('includeGlobs') || [];
            var tempExcludeGlobs =
                context.workspaceState.get('excludeGlobs') || [];

            if (config.shouldUseBuiltInFileExcludes()) {
                excludeGlobs = addGlobs(
                    vscode.workspace.getConfiguration('files.exclude'),
                    excludeGlobs
                );
            }
    ...
    function collapse() {
        context.workspaceState
            ...
            .then(clearExpansionStateAndRefresh);
    }
    function expand() {
        context.workspaceState
    ...
    function addTag(tag) {
        ...
        if (tags.indexOf(tag) === -1) {
            ...
            vscode.workspace
                .getConfiguration('todo-tree.general')
                .update('tags', tags, true);
        }
    }

    function addTagDialog() {
        vscode.window
            .showInputBox({ prompt: 'New tag', placeHolder: 'e.g. FIXME' })
            .then(function (tag) {
                if (tag) {
                    addTag(tag);
                }
    ...
    function scanWorkspaceAndOpenFiles() {
        vscode.workspace
            ...
            .update(
                'scanMode',
                SCAN_MODE_WORKSPACE_AND_OPEN_FILES,
    ...
    function dumpFolderFilter() {
        debug(
            'Folder filter include:' +
                JSON.stringify(context.workspaceState.get('includeGlobs'))
        );
        debug(
            'Folder filter exclude:' +
                JSON.stringify(context.workspaceState.get('excludeGlobs'))
        );
    }
    ...
    function checkForMarkdownUpgrade() {
        if (
            ...
            if (c.get('regex.regex').indexOf('|^\\s*- \\[ \\])') > -1) {
                ...
                var message =
                    'Todo Tree: There is now an improved method of locating markdown TODOs.';
                var buttons = [MORE_INFO_BUTTON, NEVER_SHOW_AGAIN_BUTTON];
                if (
                    c.get('regex.regex') ===
                    c.inspect('regex.regex').defaultValue
                ) {
                    message +=
                        ' Would you like to update your settings automatically?';
                    ...
                    .then(function (button) {
                        ...
                        if (button === undefined) {
                            ...
                            vscode.env.openExternal(
                                vscode.Uri.parse(
                                    'https://github.com/Gruntfuggly/todo-tree#markdown-support'
                                )
                            );
    ...
    function register() {
        function migrateSettings() {
            function migrateIfRequired(setting, type, destination) {
                ...
                if (typeMatch(details.workspaceFolderValue, type)) {
                    debug(
                        "Migrating workspaceFolder setting '" + setting + "'"
                    );
                    c.update(
                        destination + '.' + setting,
            ...
            migrateIfRequired(
                'sortTagsOnlyViewAlphabetically',
                'boolean',
                'tree'
            );
            ...
            var currentSchemes = vscode.workspace
                .getConfiguration('todo-tree.highlights')
                .get('schemes');
            if (
                ...
                var schemesSettings = vscode.workspace
                    .getConfiguration('todo-tree.general')
                    .inspect('schemes');
                ...
                if (currentSchemes !== schemesSettings.defaultValue) {
                    ...
                    vscode.workspace
                        .getConfiguration('todo-tree.general')
                        .update('schemes', currentSchemes, target);
                }
            }
        }
        ...
        function documentChanged(document) {
            if (document) {
                ...
                if (
                    config.isValidScheme(document.uri) &&
                    path.basename(document.fileName) !== 'settings.json'
                ) {
        ...
        function validateColours() {
            ...
            if (invalidColourMessage) {
                vscode.window.showWarningMessage(
                    'Todo Tree: ' + invalidColourMessage
                );
            }
            ...
            if (invalidIconColourMessage) {
                vscode.window.showWarningMessage(
                    'Todo Tree: ' + invalidIconColourMessage
                );
            }
        }
        ...
        function validatePlaceholders() {
            ...
            if (unexpectedPlaceholders.length > 0) {
                vscode.window.showErrorMessage(
                    'Todo Tree: Unexpected placeholders (' +
                        unexpectedPlaceholders.join(',') +
                        ')'
                );
            }
        }
        ...
        function shouldRefreshFile() {
            return (
                vscode.workspace.getConfiguration('todo-tree.tree')
                    .autoRefresh === true &&
        ...
        context.subscriptions.push(
            vscode.commands.registerCommand('todo-tree.filter', function () {
                vscode.window
                    .showInputBox({ prompt: 'Filter tree' })
                    .then(function (term) {
                        currentFilter = term;
                        if (currentFilter) {
        ...
        context.subscriptions.push(
            vscode.commands.registerCommand(
                ...
                function () {
                    var exportPath = vscode.workspace
                        .getConfiguration('todo-tree.general')
                        .get('exportPath');
            ...
            vscode.commands.registerCommand(
                ...
                function () {
                    ...
                    if (!config || config.length === 0) {
                        ...
                        var items = [];
                        var currentIncludeGlobs = JSON.stringify(
                            context.workspaceState.get('includeGlobs') || []
                        );
                        var currentExcludeGlobs = JSON.stringify(
                            context.workspaceState.get('excludeGlobs') || []
                        );
                        config.forEach(function (c) {
                            var scope = { label: c.name };
                            var includeGlobs = JSON.stringify(
                                utils.toGlobArray(c.includeGlobs)
                            );
                            var excludeGlobs = JSON.stringify(
                                utils.toGlobArray(c.excludeGlobs)
                            );
                            if (
                                currentIncludeGlobs === includeGlobs &&
                                currentExcludeGlobs === excludeGlobs
                            ) {
                        ...
                        vscode.window
                            ...
                            .then(function (scope) {
                                if (scope) {
                                    var currentConfig = config.find(
                                        (c) => c.name === scope.label
                                    );

                                    context.workspaceState.update(
                                        'includeGlobs',
                                        utils.toGlobArray(
                                            currentConfig.includeGlobs
                                        )
                                    );
                                    context.workspaceState.update(
                                        'excludeGlobs',
                                        utils.toGlobArray(
                                            currentConfig.excludeGlobs
                                        )
                                    );
        ...
        context.subscriptions.push(
            vscode.commands.registerCommand(
                ...
                function (node) {
                    var rootNode = tree.locateWorkspaceNode(node.fsPath);
                    var glob = utils.createFolderGlob(
                        node.fsPath,
                        rootNode.fsPath,
                        '/**/*'
                    );
                ...
                function () {
                    ...
                    excludeGlobs.forEach(function (excludeGlob) {
                        if (excludeGlob.endsWith('/**/*')) {
                            choices[
                                'Exclude Folder: ' + excludeGlob.slice(0, -5)
                            ] = { exclude: excludeGlob };
                        } else if (excludeGlob.indexOf('*') === -1) {
                            choices['Exclude File: ' + excludeGlob] = {
                                exclude: excludeGlob,
                            };
                        } else {
                            choices['Exclude: ' + excludeGlob] = {
                                exclude: excludeGlob,
                            };
                        }
                    });
                        ...
                        if (includeGlob.endsWith('/**/*')) {
                            choices[
                                'Include Folder and Subfolders: ' +
                                    includeGlob.slice(0, -5)
                            ] = { include: includeGlob };
                        } else if (includeGlob.endsWith('/*')) {
                            choices[
                                'Include Folder: ' + includeGlob.slice(0, -2)
                            ] = { include: includeGlob };
                        } else {
                            choices['Include: ' + includeGlob] = {
                                include: includeGlob,
                            };
                        }
                    });
                    ...
                    vscode.window
                        .showQuickPick(Object.keys(choices), {
                            matchOnDetail: true,
                            matchOnDescription: true,
                            canPickMany: true,
                        ...
                        .then(function (selection) {
                            if (selection) {
                                if (
                                    selection.indexOf(CLEAR_TREE_FILTER) === 0
                                ) {
                                    clearTreeFilter();
                                    selection.shift();
                                }
                                ...
                                selection.map(function (choice) {
                                    if (choices[choice].include) {
                                        includeGlobs = includeGlobs.filter(
                                            (f) => choices[choice].include != f
                                        );
                                    } else if (choices[choice].exclude) {
                                        excludeGlobs = excludeGlobs.filter(
                                            (f) => choices[choice].exclude != f
                                        );
                                    }
                                });
        ...
        context.subscriptions.push(
            vscode.commands.registerCommand(
                ...
                function () {
                    var current = vscode.workspace
                        .getConfiguration('todo-tree.tree')
                        .get('disableCompactFolders');
                    vscode.workspace
                        .getConfiguration('todo-tree.tree')
                        .update(
                            'disableCompactFolders',
                            !current,
                            vscode.ConfigurationTarget.Workspace
                        );
                }
        ...
        context.subscriptions.push(
            vscode.commands.registerCommand('todo-tree.refresh', rebuild)
        );
        context.subscriptions.push(
            vscode.commands.registerCommand(
        ...
        context.subscriptions.push(
            vscode.commands.registerCommand('todo-tree.expand', expand)
        );
        context.subscriptions.push(
            vscode.commands.registerCommand('todo-tree.collapse', collapse)
        );
        context.subscriptions.push(
            vscode.commands.registerCommand('todo-tree.groupByTag', groupByTag)
        );
        context.subscriptions.push(
            vscode.commands.registerCommand(
        ...
        context.subscriptions.push(
            vscode.commands.registerCommand('todo-tree.addTag', addTagDialog)
        );
        context.subscriptions.push(
            vscode.commands.registerCommand(
                'todo-tree.removeTag',
            ...
            vscode.window.onDidChangeActiveTextEditor(function (e) {
                if (e && e.document) {
                    ...
                    if (
                        ...
                        if (
                            e.document.uri &&
                            config.isValidScheme(e.document.uri)
                        ) {
                    ...
                    if (
                        e.document.fileName === undefined ||
                        isIncluded(e.document.uri)
                    ) {
                        updateStatusBarAndTitleBar();
                    }
                    ...
                    if (config.isValidScheme(document.uri)) {
                        if (
                            ...
                            tempSearchList.map(function (p) {
                                if (
                                    document.fileName === p ||
                                    document.fileName.indexOf(p + path.sep) ===
                                        0
                                ) {
                                    keep = true;
                                }
            ...
            vscode.workspace.onDidChangeConfiguration(function (e) {
                if (
                    ...
                    if (
                        ...
                        e.affectsConfiguration('todo-tree.tree.labelFormat')
                    ) {
                        validatePlaceholders();
                    } else if (
                        e.affectsConfiguration('todo-tree.general.debug')
                    ) {
                        resetOutputChannel();
                    } else if (
                        e.affectsConfiguration(
        ...
        if (
            ...
            editors.map(function (editor) {
                if (
                    editor.document &&
                    config.isValidScheme(editor.document.uri)
                ) {
                    openDocuments[editor.document.uri.toString()] =
                        editor.document;
                }
                refreshOpenFiles();
            });
