...
function hexToRgba(hex, opacity) {
    ...
    if (hex !== undefined) {
        ...
        var g = toComponent(
            rgb.substring(rgb.length / 3, (2 * rgb.length) / 3)
        );
        var b = toComponent(
            rgb.substring((2 * rgb.length) / 3, (3 * rgb.length) / 3)
        );

        if (hex.length == 4 || hex.length == 8) {
            opacity = parseInt(
                (toComponent(
                    hex.substring((3 * hex.length) / 4, (4 * hex.length) / 4)
                ) *
                    100) /
                    255
            );
...
function removeBlockComments(text, fileName) {
    ...
    if (
        commentPattern &&
        commentPattern.multiLineComment &&
        commentPattern.multiLineComment.length > 0
    ) {
        ...
        if (commentPattern && commentPattern.regex) {
            ...
            if (commentMatch) {
                for (
                    var i = commentPattern.cg.contentStart;
...
function extractTag(text, matchOffset) {
    ...
    if (c.regex.indexOf('$TAGS') > -1) {
        ...
        if (tagMatch) {
            ...
            var rightOfTagText = text
                .substr(tagMatch.index + tagMatch[0].length)
                .trim();
            ...
            if (rightOfTag.length === 0) {
                text = text
                    .substr(0, matchOffset ? matchOffset - 1 : tagMatch.index)
                    .trim();
                after = '';
                before = text;
            } else {
...
function isIncluded(name, includes, excludes) {
    ...
    var included =
        posix_includes.length === 0 || micromatch.isMatch(name, posix_includes);
...
function formatLabel(template, node, unexpectedPlaceholders) {
    ...
    var re = new RegExp(
        '\\$\\{(' + Object.keys(formatLabelMap).join('|') + ')\\}',
        'gi'
    );
...
function isHidden(filename) {
    return (
        path.basename(filename).indexOf('.') !== -1 &&
...
function isValidColour(colour) {
    if (colour) {
        if (
            isNamedColour(colour) ||
            isThemeColour(colour) ||
            isHexColour(colour) ||
            isRgbColour(colour)
        ) {
            return true;
