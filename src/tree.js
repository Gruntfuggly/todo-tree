...
var findTagNode = function (node) {
    ...
    return (
        isPathNode(node) &&
        node.tag &&
        node.tag.toLowerCase() === this.toString().toLowerCase()
    );
};
...
var findSubTagNode = function (node) {
    ...
    return (
        node.type === PATH &&
        node.subTag &&
        node.subTag.toLowerCase() === this.toString().toLowerCase()
    );
};
...
function createPathNode(folder, pathElements, isFolder, subTag) {
    ...
    var fsPath =
        pathElements.length > 0
            ? path.join(folder, pathElements.join(path.sep))
            : folder;
...
function createFlatNode(fsPath, rootNode) {
    var id = buildCounter * 1000000 + nodeCounter++;
    var pathLabel = path.dirname(
        rootNode === undefined ? fsPath : path.relative(rootNode.fsPath, fsPath)
    );
...
function createTodoNode(result) {
    ...
    var label =
        extracted.withoutTag && extracted.withoutTag.length > 0
            ? extracted.withoutTag
            : 'line ' + result.line;
...
function locateWorkspaceNode(filename) {
    ...
    nodes.map(function (node) {
        var workspacePath =
            node.fsPath +
            (node.fsPath.indexOf(path.sep) === node.fsPath.length - 1
                ? ''
                : path.sep);
        if (
            node.isWorkspaceNode &&
            (filename === node.fsPath || filename.indexOf(workspacePath) === 0)
        ) {
            result = node;
        }
    });
...
function locateFlatChildNode(rootNode, result, tag, subTag) {
    ...
    if (config.shouldGroupByTag() && tag) {
        ...
        if (parentNode === undefined) {
            parentNode = createPathNode(
                rootNode ? rootNode.fsPath : JSON.stringify(result),
                [tagPath],
                subTag
            );
    ...
    var fullPath =
        result.uri.scheme === 'file'
            ? result.uri.fsPath
            : path.join(result.uri.authority, result.uri.fsPath);
...
function countTags(child, tagCounts, forStatusBar, fileFilter) {
    function countTag(node) {
        if (isTodoNode(node)) {
            ...
            if (
                isVisible(node) &&
                (!fileFilter || fileFilter === node.fsPath)
            ) {
                if (!forStatusBar || !config.shouldHideFromStatusBar(tag)) {
                    tagCounts[tag] =
                        tagCounts[tag] === undefined ? 1 : tagCounts[tag] + 1;
                }
            }
        }
    }
...
class TreeNodeProvider {
    ...
    getChildren(node) {
        if (node === undefined) {
            ...
            var filterStatusNode = {
                label: '',
                notExported: true,
                isStatusNode: true,
            };
            var includeGlobs = utils.toGlobArray(
                this._context.workspaceState.get('includeGlobs')
            );
            var excludeGlobs = utils.toGlobArray(
                this._context.workspaceState.get('excludeGlobs')
            );
    ...
    getTreeItem(node) {
        ...
        try {
            treeItem = new vscode.TreeItem(
                node.label + (node.pathLabel ? ' ' + node.pathLabel : '')
            );
        } catch (e) {
        ...
        if (node.fsPath) {
            ...
            if (isTodoNode(treeItem.node)) {
                ...
                treeItem.tooltip = utils.formatLabel(
            ...
            if (isPathNode(node)) {
                if (config.shouldCompactFolders() && node.tag === undefined) {
                    var onlyChild =
                        node.nodes.filter(isPathNode).length === 1
                            ? node.nodes[0]
                            : undefined;
                ...
                if (node.tag) {
                    treeItem.iconPath = icons.getIcon(
                        this._context,
                        node.tag ? node.tag : node.label,
                        this._debug
                    );
                ...
                if (
                    ...
                    (config.shouldGroupByTag() !== true &&
                        config.shouldGroupBySubTag() !== true)
                ) {
                ...
                if (
                    format !== '' &&
                    (node.extraLines === undefined ||
                        node.extraLines.length === 0)
                ) {
                    treeItem.label =
                        utils.formatLabel(format, node) +
                        (node.pathLabel ? ' ' + node.pathLabel : '');
                }

                var revealBehaviour = vscode.workspace
                    .getConfiguration('todo-tree.general')
                    .get('revealBehaviour');

                var todoSelection;
                if (revealBehaviour === 'end of todo') {
                    var todoEnd = new vscode.Position(
                        node.line,
                        node.endColumn - 1
                    );
                    ...
                    var todoStart = new vscode.Position(
                        node.line,
                        node.column - 1
                    );
        ...
        if (config.shouldShowCounts() && isPathNode(node)) {
            ...
            var total = Object.values(tagCounts).reduce(function (a, b) {
                return a + b;
            }, 0);
    ...
    filter(text, children) {
        var matcher = new RegExp(
            text,
            config.showFilterCaseSensitive() ? '' : 'i'
        );
        ...
        children.forEach((child) => {
            ...
            if (
                ...
                (child.extraLines && child.extraLines.length > 0)
            ) {
                var visibleNodes = child.nodes
                    ? child.nodes.filter(isVisible).length
                    : 0;
                var visibleExtraLines = child.extraLines
                    ? child.extraLines.filter(isVisible).length
                    : 0;
    ...
    add(result) {
        ...
        if (
            config.shouldHideFromTree(
                todoNode.tag ? todoNode.tag : todoNode.label
            )
        ) {
        ...
        var tagPath = todoNode.subTag
            ? todoNode.tag + ' (' + todoNode.subTag + ')'
            : todoNode.tag;
    ...
    reset(uri, children) {
        var fullPath =
            uri.scheme === 'file'
                ? uri.fsPath
                : path.join(uri.authority, uri.fsPath);
        ...
        children = children.filter(function (child) {
            ...
            if (child.type === TODO && !child.tag && child.fsPath == fullPath) {
                // no tag (e.g. markdown)
                keep = false;
            } else if (
                child.type === TODO &&
        ...
        function removeNodesByFilename(children, me) {
            return children.filter(function (child) {
                ...
                if (shouldRemove) {
                    ...
                    me._context.workspaceState.update(
                        'expandedNodes',
                        expandedNodes
                    );
                    if (callback) {
    ...
    exportChildren(parent, children) {
        children.forEach(function (child) {
            if (child.type === PATH) {
                ...
                parent[itemLabel] =
                    format !== ''
                        ? utils.formatLabel(format, child) +
                          (child.pathLabel ? ' ' + child.pathLabel : '')
                        : child.label;
            }
        }, this);
        return parent;
