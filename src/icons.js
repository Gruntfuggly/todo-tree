...
function getIcon(context, tag, debug) {
    ...
    var darkIconPath = context.asAbsolutePath(
        path.join('resources/icons', 'dark', 'todo-green.svg')
    );
    var lightIconPath = context.asAbsolutePath(
        path.join('resources/icons', 'light', 'todo-green.svg')
    );

    var compactColourName = colour;
    ...
    if (
        ...
        if (debug) {
            debug(
                'Attempting to create global storage folder ' +
                    context.globalStorageUri.fsPath
            );
        }
    ...
    if (iconName === 'todo-tree' || iconName === 'todo-tree-filled') {
        ...
        if (!fs.existsSync(colouredTodoTreeIconPath)) {
            ...
            fs.writeFileSync(
                colouredTodoTreeIconPath,
                colouredTodoTreeIconDefinition
            );
        }
        ...
        return new vscode.ThemeIcon(
            iconName.trim().substr(2, iconName.trim().length - 3),
            themeColour
        );
    } else if (iconName) {
        ...
        if (context.globalStorageUri.fsPath) {
            var octiconIconPath = path.join(
                context.globalStorageUri.fsPath,
                'todo-' + iconName + '-' + compactColourName + '.svg'
            );
            if (!fs.existsSync(octiconIconPath)) {
                var octiconIconDefinition =
        ...
        utils.isHexColour(colour) ||
        utils.isRgbColour(colour) ||
        utils.isNamedColour(colour)
    ) {
        var colouredIconPath = path.join(
            context.globalStorageUri.fsPath,
            'todo-' + compactColourName + '.svg'
        );
    ...
    function checkIcon(setting) {
        ...
        if (icon !== undefined) {
            if (utils.isCodicon(icon)) {
                ...
                if (
                    codiconNames.indexOf(codicon) === -1 &&
                    productIconNames.indexOf(codicon) === -1
                ) {
