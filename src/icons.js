...
function getIcon( context, tag, debug )
{
    ...
    if( context.globalStorageUri.path && !fs.existsSync( context.globalStorageUri.path ) )
    {
        if( debug )
        {
            debug( "Attempting to create global storage folder " + context.globalStorageUri.path );
        }
    ...
    if( iconName === 'todo-tree' || iconName === 'todo-tree-filled' )
    {
        ...
        var colouredTodoTreeIconPath = path.join( context.globalStorageUri.path, iconName + "-" + compactColourName + ".svg" );
    ...
    else if( iconName )
    {
        ...
        {
            var octiconIconPath = path.join( context.globalStorageUri.path, "todo-" + iconName + "-" + compactColourName + ".svg" );
    ...
    {
        var colouredIconPath = path.join( context.globalStorageUri.path, "todo-" + compactColourName + ".svg" );
