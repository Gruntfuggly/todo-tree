    ...
    if (editor) {
        ...
        while ((match = regex.exec(text)) !== null) {
            ...
            if (type !== 'none') {
                ...
                if (type === 'block') {
                    let idLine = startPos.line;
                    while (editor.document.lineAt(idLine).text.indexOf('*/') < 0) {
                        idLine++;
                    }
                    endPos = new vscode.Position(idLine, editor.document.lineAt(idLine).range.end.character);
